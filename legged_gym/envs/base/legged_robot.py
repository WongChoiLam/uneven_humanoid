...
class LeggedRobot(BaseTask):
    ...
    def _reward_dof_pos_limits(self):
        ...
        out_of_limits = -(self.dof_pos[:, :12] - self.dof_pos_limits[:12, 0]).clip(max=0.) # lower limit
        out_of_limits += (self.dof_pos[:, :12] - self.dof_pos_limits[:12, 1]).clip(min=0.)
    ...
    def _reward_stand_still(self):
        ...
        return torch.sum(torch.abs(self.dof_pos[:, :12] - self.default_dof_pos[:, :12]), dim=1) * (torch.norm(self.commands[:, :2], dim=1) < 0.1)
