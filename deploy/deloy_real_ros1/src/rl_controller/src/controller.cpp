...
{
    YAML::Node yaml_node = YAML::LoadFile(ros::package::getPath("rl_controller")+"/configs/g1.yaml");
	...
	module = torch::jit::load(ros::package::getPath("rl_controller")+"/models/motion.pt");
...
{
	state_pub = nh.advertise<robot_msgs::RobotState>("/state", 1);
	cmd_pub = nh.advertise<robot_msgs::RobotCommand>("/cmd", 1);
	return;
}
...
void Controller::move_to_default_pos()
{
	...
	while (count <= num_steps || !joy.btn.components.A) 
	{
		...
		//! xulong: C++14
		float phase = std::max(std::min(float(count++) / num_steps, 1.0f), 0.0f);
...
void Controller::run()
{
	...
	while (!joy.btn.components.select)
	{
		...
		robot_msgs::IMU imu_msg;
		robot_msgs::MotorState motor_state_msg;

		for (int i = 0; i < 3; i++)
		{
			imu_msg.quaternion[i] = low_state->imu_state().quaternion()[i];
			imu_msg.gyroscope[i] = low_state->imu_state().gyroscope()[i];
			imu_msg.accelerometer[i] = low_state->imu_state().accelerometer()[i];
		}
		imu_msg.quaternion[3] = low_state->imu_state().quaternion()[3];
		state_msg.imu = imu_msg;
		for (int i=0; i<29; i++)
		{
			motor_state_msg.q = low_state->motor_state()[i].q();
			motor_state_msg.dq = low_state->motor_state()[i].dq();
			// motor_state_msg.cur = low_state->motor_state()[i].tau();
			motor_state_msg.ddq = low_state->motor_state()[i].ddq();
			motor_state_msg.tau_est = low_state->motor_state()[i].tau_est();
			state_msg.motor_state.push_back(motor_state_msg);
		}
		...
		// ros debug
		robot_msgs::RobotCommand cmd_msg;
		robot_msgs::MotorCommand cmd_motor;
		...
		for (int i = 0; i < 12; i++)
		{
			...
			cmd_motor.q = low_cmd->motor_cmd()[i].q();
			cmd_motor.dq = low_cmd->motor_cmd()[i].dq();
			cmd_motor.kp = low_cmd->motor_cmd()[i].kp();
			cmd_motor.kd = low_cmd->motor_cmd()[i].kd();
			cmd_motor.tau = low_cmd->motor_cmd()[i].tau();
			cmd_msg.motor_command.push_back(cmd_motor);
		}
...
void Controller::low_cmd_write_handler()
{
	if (auto lowCmdPtr = mLowCmdBuf.GetDataPtr())
	{
		...
		lowCmdPtr->crc() = crcCore<unsigned int>((uint32_t*)(lowCmdPtr.get()), (sizeof(unitree_hg::msg::dds_::LowCmd_) >> 2) - 1);
